 ...
 * ACP, k-means, corrélation de Pearson, régression linéaire
 */
...
const Stats = (() => {
  ...
  /** Centre et réduit chaque colonne (z-score) */
  ...
  /** Matrice de covariance d'une matrice déjà centrée */
  ...
  /**
   * Jacobi eigendecomposition pour matrices symétriques.
   * Retourne { vectors, values } triés par valeur propre décroissante.
   */
  ...
  /**
   * ACP principale.
   * @param {Array<Object>} data - tableau d'objets avec les variables numériques
   * @param {string[]} variables - noms des colonnes à utiliser
   * @returns {{ scores, loadings, variance, explainedRatio }}
   */
  function pca(data, variables) {
    ...
    // Scores (projections des individus)
    ...
    return {
      scores,
      loadings: vectors,    // vecteurs propres [composante][variable]
      variance: values,
      explainedRatio,
      variables,
    };
  ...
  /**
   * K-means clustering.
   * @param {number[][]} points - tableau de vecteurs numériques
   * @param {number} k
   * @param {number} maxIter
   * @returns {{ labels, centroids, inertia }}
   */
  function kmeans(points, k, maxIter = 100) {
    // Initialisation k-means++ (approximation : choisir k premiers points distincts)
    ...
    for (let iter = 0; iter < maxIter; iter++) {
      ...
      labels.forEach((l, i) => {
        points[i].forEach((v, d) => { sums[l][d] += v; });
        counts[l]++;
      });
      centroids = sums.map((s, ki) =>
        counts[ki] > 0 ? s.map(v => v / counts[ki]) : centroids[ki]
      );
    }

    const inertia = points.reduce((s, p, i) =>
      s + euclidean(p, centroids[labels[i]]) ** 2, 0);

    return { labels, centroids, inertia };
  }

  /**
   * Méthode du coude : calcule l'inertie pour k de 2 à maxK.
   */
  ...
  /**
   * Coefficient de Pearson + p-value (approximation t-distribution).
   * @returns {{ r, pvalue, n }}
   */
  ...
  /** Approximation de la CDF de Student via régularisée beta (Abramowitz & Stegun) */
  function tCDF(t, df) {
    const x = df / (df + t * t);
    const a = df / 2, b = 0.5;
  ...
  /**
   * Matrice de corrélation complète pour un tableau d'objets.
   * @param {Array<Object>} data
   * @param {string[]} variables
   * @returns {Array<Array<{r, pvalue, n}>>}
   */
  ...
  /**
   * Régression linéaire simple y = a + b*x (moindres carrés).
   * @returns {{ a, b, r2, pvalue, n, se, xMean, yMean, residuals }}
   */
  function linearRegression(x, y) {
    ...
    // IC 95% pour la droite de régression
    const xRange = Array.from({ length: 50 }, (_, i) => {
      const xmin = Math.min(...x), xmax = Math.max(...x);
      return xmin + (xmax - xmin) * i / 49;
    });

    const tCrit = 1.96; // approximation pour grands n; ou t(n-2, 0.975)
  ...
  return { pca, kmeans, elbowData, pearson, correlationMatrix, linearRegression, mean, std, standardize };
